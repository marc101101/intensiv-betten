...
export class CollectIntensivBettenClient {
  ...
  public async collectIntensivBettenRegister(
    url,
    method
  ): Promise<Array<diviEntry>> {
    this.url = url;
    this.method = method;
    this.setOptions();
  ...
  public async collectIntensivBettenCapacity(
    url,
    method
  ): Promise<Array<diviEntry>> {
    this.url = url;
  ...
  private extractTableData() {
    ...
    let data = this.root.querySelectorAll("script");
    data = data[data.length - 1].structuredText;
    data = data.split("vegaEmbed")[1];

    let info = data.split("data-");
    info = info[5];
    info = info.substring(35, info.length - 3);
