...
.PHONY: sls-deploy
sls-deploy: ## Deploys serverless lambda
	npx sls deploy --stage=$(STAGE)

.PHONY: sls-remove
sls-remove: ## Remove serverless lambda
	npx sls remove --stage=$(STAGE)

.PHONY: invoke
invoke: ## Invoke given function remotely
ifndef FN
	$(error Argument FN must be provided and filled with the function name)
endif
	npx sls invoke --function ${FN}

.PHONY: invoke-local
invoke-local: ## Invoke given function locally
ifndef FN
	$(error Argument FN must be provided and filled with the function name)
endif
	npx sls invoke local --function ${FN}
